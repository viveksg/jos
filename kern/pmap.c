...
mem_init_mp(void)
{
    ...
    int i = 0;
	uint32_t cpu_stack_virtual = KSTACKTOP;
	uint32_t stack_offset = KSTKSIZE + KSTKGAP;
	for( i = 0; i < NCPU; i++ , cpu_stack_virtual-= stack_offset)
	{
        boot_map_region(kern_pgdir,cpu_stack_virtual - KSTKSIZE, KSTKSIZE , PADDR(percpu_kstacks[i]), PTE_W);
	}
}
...
page_init(void)
{
	...
	uint32_t mpentery = MPENTRY_PADDR / PGSIZE;
	for (i = 0; i < npages; i++) {
		if(i == 0 || i == mpentery || (i >= iophysmem_first_pg && i < kernel_boot_pg_limit) ){
		   pages[i].pp_ref = 1;
		   continue;
		}
		pages[i].pp_ref = 0;
...
{   
	...
	char* page_address = (char*) page2kva(freepage);
	if(alloc_flags & ALLOC_ZERO)
	     memset(page_address, 0, PGSIZE); 
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	uintptr_t pgdir_offset = PDX(va);
	uintptr_t pte_offset = PTX(va);
	pde_t *pgdir_entry = &pgdir[pgdir_offset];
    if(!(*pgdir_entry & PTE_P)){
        ...
        if(pt_page == NULL)
		    return NULL;
		pt_page->pp_ref++;
		*pgdir_entry = page2pa(pt_page)|PTE_U|PTE_W|PTE_P;	 
	}
	pte_t *pt_base = (pte_t *)KADDR(PTE_ADDR(*pgdir_entry));
	return &(pt_base[pte_offset]);
}
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	// Fill this function in
	pte_t * pt_entry; 
	if((pt_entry = pgdir_walk(pgdir, va, 1)) == NULL)
	   return -E_NO_MEM;
	pp->pp_ref++;  
	if(*pt_entry & PTE_P)
...
{
    pte_t *pt_entry ;
    if((pt_entry = pgdir_walk(pgdir, va, 0)) == NULL || !((*pt_entry) & PTE_P))
         return NULL;
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	pte_t * pt_entry;
	struct PageInfo * pp_info =  page_lookup(pgdir, va, &pt_entry);
	if(pp_info){
		page_decref(pp_info);
		*pt_entry = 0x0;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if(base + region_size >= MMIOLIM){
		panic("mmio_map_region: requested size beyond memory mapped io limit");
	}
	boot_map_region(kern_pgdir, base, region_size, pa, PTE_PCD | PTE_PWT | PTE_W);
	void * curr_base = (void*)base;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t curr_va = ROUNDDOWN((uintptr_t)va, PGSIZE);
	uintptr_t va_end =  ROUNDUP((uintptr_t)(va + len), PGSIZE);
	uint32_t final_perm = PTE_P | perm;
	while(curr_va < va_end){
		pte_t * pt_entry = pgdir_walk(env_pgdir, (const void*)curr_va, 0);
		if(curr_va >= ULIM || pt_entry == NULL || (*pt_entry & final_perm) != final_perm){
			user_mem_check_addr = curr_va < va_start? va_start: curr_va;
			return -E_FAULT;
		}
